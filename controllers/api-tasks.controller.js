...
async function getToDos(req, res, next) {
  ...
  if (stat.employee_status === "Сотрудник") {
    ...
    todoList.forEach(item => {
      ...
      item.idAuthor = Author.last_name_employee +' '+ Author.name_employee + ' '+Author.patronymic_employee
      item.idExecutor = Executor.last_name_employee +' '+ Executor.name_employee +' '+ Executor.patronymic_employee
    })
...
async function patchToDo(req, res, next) {
  ...
  if (stat.employee_status === "Сотрудник") {
    ...
    if (Keys.includes("idExecutor")) {
      if (empl.includes(req.body.idExecutor) || req.body.idExecutor === req.userId) {
        todo = await todo.update(req.body, { returning: true });
      } else {
        throw new ErrorResponse(
          "Нет прав на назначение задачи другому менеджеру!",
